...
function FileUpload({ label, description, preview, onUpload, onRemove, required }) {
  ...
  return (<div><label style={{ display: "block", fontSize: 12, fontWeight: 700, color: C.gray600, marginBottom: 5, fontFamily: "'Lato', sans-serif", textTransform: "uppercase", letterSpacing: "0.04em" }}>{label} {required && <span style={{ color: C.sage }}>*</span>}</label>{description && <p style={{ fontSize: 12, color: C.gray400, fontFamily: "'Lato', sans-serif", marginBottom: 10, lineHeight: 1.5 }}>{description}</p>}{!preview ? (<div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><label style={{ flex: "1 1 45%", minWidth: 150, padding: "18px 14px", background: C.cream, border: "2px dashed " + C.gray200, borderRadius: 10, cursor: "pointer", textAlign: "center" }}><input type="file" accept="image/*" style={{ position: "absolute", width: 1, height: 1, opacity: 0, overflow: "hidden" }} onChange={handleFile} /><div style={{ fontSize: 26, marginBottom: 4 }}>{"\uD83D\uDCF8"}</div><div style={{ fontSize: 13, fontWeight: 700, color: C.charcoal, fontFamily: "'Lato', sans-serif" }}>Take Photo / Choose Photo</div><div style={{ fontSize: 11, color: C.gray400, fontFamily: "'Lato', sans-serif" }}>Opens camera or photo library</div></label><label style={{ flex: "1 1 45%", minWidth: 150, padding: "18px 14px", background: C.cream, border: "2px dashed " + C.gray200, borderRadius: 10, cursor: "pointer", textAlign: "center" }}><input type="file" accept="image/*,.pdf" style={{ position: "absolute", width: 1, height: 1, opacity: 0, overflow: "hidden" }} onChange={handleFile} /><div style={{ fontSize: 26, marginBottom: 4 }}>{"\uD83D\uDCC1"}</div><div style={{ fontSize: 13, fontWeight: 700, color: C.charcoal, fontFamily: "'Lato', sans-serif" }}>Upload File</div><div style={{ fontSize: 11, color: C.gray400, fontFamily: "'Lato', sans-serif" }}>JPG, PNG, or PDF</div></label></div>) : (<div style={{ position: "relative", display: "inline-block" }}>{typeof preview === "string" && preview.startsWith("data:image") ? (<img src={preview} alt="Preview" style={{ maxWidth: "100%", maxHeight: 180, borderRadius: 10, border: "2px solid " + C.sage, objectFit: "contain" }} />) : (<div style={{ padding: "16px 20px", background: C.cream, border: "1px solid " + C.sage, borderRadius: 10, display: "flex", alignItems: "center", gap: 12 }}><span style={{ fontSize: 26 }}>{"\uD83D\uDCC4"}</span><div><div style={{ fontSize: 13, fontWeight: 700, color: C.charcoal, fontFamily: "'Lato', sans-serif" }}>{typeof preview === "string" ? preview : "Uploaded"}</div><div style={{ fontSize: 11, color: C.green, fontFamily: "'Lato', sans-serif" }}>{"\u2713 Uploaded"}</div></div></div>)}<button onClick={onRemove} style={{ position: "absolute", top: -8, right: -8, width: 24, height: 24, borderRadius: "50%", background: C.red, color: "#fff", border: "none", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700 }}>{"\u00D7"}</button></div>)}</div>);
}
